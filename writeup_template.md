# The Estimation Project Writeup #
...
I have calculated the mean and standard deviation using a Matlab file "mean_std_calc.m" that you can find in the top directory! You need to change the path of the sensor files in the Matlab script!
...
The attitude estimation code goes like:
...
Here I have used Quaternion. Integration of Gyro data (= angular r-p-y 'rates' in body frame), gives r-p-y angle. the yaw angle is normalized to be within +pi to -pi.
...
In the prediction step, the system dynamics/kinematics [in general, dynamics] basically accumulates the error (for example, elliptically in X-Y) in the process and due to erroneous measurement in the states and also in the "control input" measurement (new ;) )!
...
This is just the measurement from the magnetometer which is basically the yaw angle! this contains error in measurement and the error depends on the standard deviation parameter in the QuadEstimatorEKF.txt file!
...
This function collects the position of the quad in the inertial frame [utm coordinate], and is erroneous which is given by the gps standard deviation in quaEstimatorEKF.txt
...
I have added my controller and the things are all working fine with the following parameters
...
Even my controller is working fine with the following gain parameters
...
Thanks to the control project reviewer!


Also thanks to you for your review! The key is to make the controller project right! But with the default controller, I am not able to get the right 'QYawStd' value to pass Yaw error scenario! Can you give me some clue? or can you directly give me the value for the default controller to pass the yaw error scenario?
